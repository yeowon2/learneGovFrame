...
//		Scribe에서 제공하는 인증 URL 생성 기능을 이용하여 네아로(네이버아이디로 로그인) 인증 URL 생성
...
//		Callback으로 전달받은 세션 검증을 난수값과 세션에 저장되어 있는 값이 일치하는지 확인
		...
		if(StringUtils.pathEquals(sessionState, state)) {// 이 때 StringUtils는 springframework 받기 
...
//			Scribe에서 제공하는 AccessToken 획득 기능으로 네아로(네이버아이디로 로그인) AccessToken을 획득
