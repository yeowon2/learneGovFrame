...
<h2 class="icon1">회원정보 입력</h2>
<p class="mB20">회원 가입을 위해 아래의 정보를 입력해주십시오. <br class="m-block" /><strong class="star">*</strong>표시는 필수 입력사항입니다.</p>
			...
			<tr>
				...
				<td>
					...
					<select id="domain">
						...
						<option value="gamail.com">구글(G메일)</option>
...
function regist(){
	...
	if($("#password").val()){
		var pw = $("#password").val(),
			...
			// 대소문자, 특수문자, 숫자는 10자리 이상 정규형
			passwordRules2 = /^(?=.*[a-zA-Z])(?=.*[0-9]).{10,30}$/,
			result = false;
			
		if(pw.length <10) {
			if(!passwordRules1.test(pw)){
				message = "영문, 숫자, 특수문자 등 3가지 사용 시 8자 이상, 2가지 사용 시 10자리 이상";
			}else{
				result = true;
			}
		}
