	...
	<c:choose>
		...
		<c:when test="${step eq '2'}">
			<li>
				...
				<h4>기본 문법 : &#60;c:import url="URL주소 값" charEncoding="utf-8"/&#62;</h4>
		...
		<c:when test="${step eq '5'}">
			<li>
				<h3>c:choose, c:when, c:otherwise Tag : 조건문 중 하나, JAVA에서 사용하는 If, else if, else와 비슷함</h3>
				<h4>기본 문법 : <br/>
					&#60;c:choose&#62; <br/>
		...
		<c:when test="${step eq '8'}">
			<li>
				...
				<a href="${link1}" target="_blank">링크1번</a><br/>
		...
		<c:when test="${step eq '9' }">
			<li>
				...
				<c:set var="str" value="지금은 fn:contain 연습중"/>
		...
		<c:when test="${step eq '17' }">
			<li>
				<h3>fn:substring() : 주어진 시작 및 끝의 위치에 따라 문자열의 하위 집합을 반환</h3>
		...
		<c:when test="${step eq '18' }">
			<li>
				<h3>fn:replac() : 모든 문자열을 다른 문자열 시퀀스로 바꿈</h3>
